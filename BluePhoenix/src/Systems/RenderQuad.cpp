...
RenderQuad::RenderQuad(GLFWwindow* window) : window(window)
{
	...
	unsigned char* data = stbi_load("../BluePhoenix/src/Textures/container.jpg", &width, &height, &nrChannels, 0);
	if (data)
	{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load("../BluePhoenix/src/Textures/awesomeface.png", &width, &height, &nrChannels, 0);
	if (data)
	{
